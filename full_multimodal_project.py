...
def extract_and_save_smarts(html_file, output_json):
    """
    Extract SMARTS patterns from HTML and save to JSON file.
    ...
    Returns:
        List of extracted SMARTS patterns with metadata
    """
...
i = torch.LongTensor(indices)  # Use LongTensor for indices
...
# Create chemicalsub_citing list
    ...
    for i in range(target.shape[1]):
        try:
            task_metrics = {
                ...
                'Positive': np.sum(target[:, i]),
                'Zero Values': np.sum(target[:, i] == 0)
            }
...
def print_per_task_metrics(metrics, set_name):
    ...
    for i, task_metrics in enumerate(metrics['Per_Task']):
        ...
        print(f"    Num_Positive: {task_metrics['Positive']}")
        print(f"    Num_Zero: {task_metrics['Zero Values']}")
...
def save_metrics_to_csv(dataset_name, task_type, metrics, filename=f"./{dataset_name}/{dataset_name}_test_metrics.csv"):
    ...
    if task_type == 'classification' and 'Per_Task' in metrics:
        for i, task_metrics in enumerate(metrics['Per_Task']):
            row = {
                ...
                'Num_Positive': task_metrics['Positive'],
                'Num_Zero': task_metrics['Zero Values']
            }
            rows.append(row)
            
            # Only include valid metrics in averages
            if not np.isnan(task_metrics['Accuracy']) and not np.isnan(task_metrics['ROC-AUC']):
                total_acc += task_metrics['Accuracy']
                total_auc += task_metrics['ROC-AUC']
                num_tasks += 1
    ...
    fieldnames = ['dataset_name', 'task_type', 'metric_type']
    if task_type == 'regression':
...
def load_hypergraph_embeddings(dataset_name):
    ...
    try:
        ...
        return embeddings  # Assuming this returns a dictionary {smiles: tensor}
    ...
    def embed_molecules(self, smiles_list, feature_type="graph+text+hypergraph"):
        ...
        for smiles in tqdm(smiles_list, desc="Embedding molecules"):
            try:
                ...
                # Initialize all feature types as None
                graph_emb = None
                # text_emb = None
                hg_emb = None
                fp = None
        ...
        # Process text embeddings if requested
...
def calculate_classification_metrics(y_true, y_pred_probs, y_pred=None):
    ...
    for i in range(y_true.shape[1]):
        try:
            ...
            metrics['Per_Task'].append({'ROC_AUC': task_auc, 'Accuracy': task_acc, 'Num_Positive': np.sum(y_true[:, i]), 'Num_Zeros': np.sum(y_true[:, i]== 0)})
    ...
    if task_type == 'classification':
        if metrics['Per_Task']:
            ...
            for i, task in enumerate(metrics['Per_Task']):
                print(f"  Task {i}: ROC-AUC: {task['ROC_AUC']:.4f}, Accuracy: {task['Accuracy']:.4f}, Num_Positive: {task['Positive']}, Num_Zero: {task['Zero Values']}")
...
def save_metrics_to_csv(dataset_name, task_type, metrics, model_name, y_true):
    ...
    with open(filename, mode='a', newline='') as csvfile:
        if task_type == 'classification':
            fieldnames = ['model', 'task_type', 'metric_type', 'ROC_AUC', 'Accuracy', 'Num_Positive', 'Num_Zero']
        else:
        ...
        if task_type == 'classification':
            if metrics['Per_Task']:
                writer.writerow({
                    ...
                    'Num_Positive': sum(t['Positive'] for t in metrics['Per_Task']),
                    'Num_Zero': sum(t['Zero Values'] for t in metrics['Per_Task'])
                })
                for i, task_metrics in enumerate(metrics['Per_Task']):
                    writer.writerow({
                        'model': model_name,
